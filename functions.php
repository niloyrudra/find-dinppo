...
// 	wp_enqueue_script('find-dinppo-customizer', get_template_directory_uri() . '/js/customizer.js', array(), null, true);
	...
	if( is_archive("faq") ) {
		wp_localize_script("find-dinppo-navigation", "ppo_data", array(
			...
			'available_spots'	=> ppo_get_daycare_user_available_spots(get_current_user_id()),
	...
	if( is_page("profile-daycare") ) {
		$daycare_user_id = $_POST && $_POST['ppo_daycare_user_id'] ? $_POST['ppo_daycare_user_id'] : null;
		wp_localize_script("find-dinppo-navigation", "ppo_data", array(
			'ajax_url'	=> esc_url(admin_url( 'admin-ajax.php' )),
			'subs_details_list'	=> ppo_all_subscription_detail(),
			'available_spots'	=> ppo_get_daycare_user_available_spots($daycare_user_id),
		));
	}
	else {
		wp_localize_script("find-dinppo-navigation", "ppo_data", array(
...
function ppo_is_admin() {
	$user_roles = ppo_get_current_user_roles();
	$is_admin = is_array($user_roles) && in_array( 'administrator', $user_roles ) ? true : false;
	return $is_admin;
}

function ppo_is_family_member() {
	$user_roles = ppo_get_current_user_roles();
	$is_family_member = is_array($user_roles) && in_array( 'family_member', $user_roles ) ? true : false;

	return $is_family_member;
}

function ppo_is_daycare() {
	$user_roles = ppo_get_current_user_roles();
...
function ppo_get_daycares($custom_args=array())
{
	$args = array(
		'role__in' => array('day_care'),
	);
	if( count($custom_args) ) {
		foreach($custom_args as $custom_arg_key => $custom_arg_value ) {
			$args[$custom_arg_key] = $custom_arg_value;
		}
	}
...
function ppo_get_premium_daycares()
{
	$args = array(
		'role__in' => array('day_care'),
// 		'meta_query'	=> array(
...
// 			'compare'		=> '=='
// 		),
		'meta_key' => '_daycare_subscription_plan',
		'meta_compare'  =>  'LIKE', // '==',
		'meta_value' => 'premium',
	);
	return get_users( $args ) ?? [];
...
function ppo_get_subscription_plan_icon( $plan_name )
{
	$icon_link = '';
	...
	switch ($plan_name) {
		case 'gratis':
		case '' :
			$icon_link = ppo_get_dir("/assets/images/member-stars/star-black.png");
			break;
		case 'basic':
			$icon_link = ppo_get_dir("/assets/images/member-stars/star-green.png");
			break;
		case 'starter':
			$icon_link = ppo_get_dir("/assets/images/member-stars/star-yellow.png");
			break;
		case 'pro':
			$icon_link = ppo_get_dir("/assets/images/member-stars/star-light-blue.png");
			break;
		case 'premium':
			$icon_link = ppo_get_dir("/assets/images/member-stars/premium.png");
			break;
		default:
			$icon_link = ppo_get_dir("/assets/images/member-stars/star-black.png");
			break;
...
// 	return @get_user_meta( $user_id, '_daycare_address', true ) ? esc_html(get_user_meta( $user_id, '_daycare_address', true )) : '';
	...
	$address = $post_code && $building ? $post_code . " " . $building : ( !$post_code && $building ? $building : ( $post_code && !$building ? $post_code : 'Ingen adresse' ) );
	return $address;
}
...
function ppo_get_daycare_user_available_spots($user_id) {
	return ($user_id && @get_user_meta( $user_id, '_daycare_user_available_spots', true )) ? json_decode(get_user_meta( $user_id, '_daycare_user_available_spots', true )) : '';
}
function ppo_get_daycare_user_keywords($user_id) {
	return ($user_id && @get_user_meta( $user_id, '_daycare_user_keywords', true )) ? esc_html(get_user_meta( $user_id, '_daycare_user_keywords', true )) : array();
}
...
// Daycare Description
function ppo_get_daycare_description($user_id) {
	return @get_the_author_meta( 'description', $user_id ) ? esc_html(get_the_author_meta( 'description', $user_id )) : '';
}


// Social Handlers
function ppo_get_daycare_website($user_id) {
	return @get_the_author_meta( 'user_url', $user_id ) ? esc_html(get_the_author_meta( 'user_url', $user_id )) : ''; // url
}
...
/* ***** */
function ppo_get_faq_positive_stats($faq_id) {
	return @get_post_meta( $faq_id, '_ppo_is_helpful', true ) ? get_post_meta( $faq_id, '_ppo_is_helpful', true ) : 0;
}
function ppo_get_faq_negative_stats($faq_id) {
	return @get_post_meta( $faq_id, '_ppo_is_not_helpful', true ) ? get_post_meta( $faq_id, '_ppo_is_not_helpful', true ) : 0;
}
...
function ppo_generate_necessary_cpt()
{
	...
	$faq_args = array(
		'labels' => $faq_labels,
		'description' => 'Displays Frequently Asked Questions and their Answers',
		'public' => true,
		'hierarchical' => false,
		'menu_position' => 4,
 ...
 * 
 * */
add_action( 'add_meta_boxes', 'ppo_register_faq_meta_boxes' );
add_action( 'save_post', 'ppo_faq_post_save' );

function ppo_register_faq_meta_boxes() {
	add_meta_box(
		'ppo_faq_meta',
        'How helpful the answer is!',
        'ppo_register_faq_meta_boxes_callback',
        'ppo_faq',
        'side' // "normal/advance/side"
	);
}
// Callbacks
function ppo_register_faq_meta_boxes_callback($post) {
	// Set metadata if already exists.
    $is_helpful = ppo_get_faq_positive_stats($post->ID);

    $is_not_helpful = ppo_get_faq_negative_stats($post->ID);
    // Output metadata fields.
    ?>
	<h4  style="color:blue;">Is it helpful?</h4>
    <p>
		<label for="is_helpful" style="color:green;">YES!</label>
    	<input type="number" name="is_helpful" id="is_helpful" class="text-fat" value="<?php echo esc_html($is_helpful); ?>">
    </p>
    <p>
		<label for="is_not_helpful" style="color:red;">NO!</label>
    	<input type="number" name="is_not_helpful" id="is_not_helpful" class="text-fat" value="<?php echo esc_html($is_not_helpful); ?>">
    </p>
    <?php
}
// Callback for saving custom metadata to the database.
function ppo_faq_post_save( $post_id ){
...
function ppo_redirects()
{
	if(is_user_logged_in()) {
		...
		if( is_page("profile-daycare") && ($_POST && $_POST['ppo_daycare_user_id'] && ( intval( get_current_user_id() ) == intval( $_POST['ppo_daycare_user_id'] ) )) ) {
			wp_redirect(home_url("/profile/"), 302);
			exit;
		}
	}
